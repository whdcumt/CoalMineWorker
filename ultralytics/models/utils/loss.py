...
class DETRLoss(nn.Module):
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, epoch, postfix=""):
        ...
        loss[name_giou] = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, epoch=epoch, UIoU=True)
    ...
    def forward(self, pred_bboxes, pred_scores, batch, epoch, postfix="", **kwargs):
        ...
        total_loss = self._get_loss(
            pred_bboxes[-1], pred_scores[-1], gt_bboxes, gt_cls, gt_groups, epoch=epoch, postfix=postfix, match_indices=match_indices
        )

        if self.aux_loss:
            total_loss.update(
                self._get_loss_aux(
                    pred_bboxes[:-1], pred_scores[:-1], gt_bboxes, gt_cls, gt_groups, epoch, match_indices, postfix,
                )
            )
